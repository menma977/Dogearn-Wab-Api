...
class SendDataToOtherDomain4 extends Command
{
  /**
   ...
   * @var string
   */
  protected $signature = 'minute:sendDataToOtherDomain4';
   ...
   * @var string
   */
  protected $description = 'Send Withdraw To domain 4';
  ...
  public function handle()
  {
    try {
      ...
      if ($data) {
        ...
        if ($data->send_to == 0) {
          $response = Http::asForm()->post('http://api4.budisetiyono.com/nembak.php', [
        ...
        if ($response->successful() && str_contains($response->body(), '1')) {
          ...
          Log::info('delivery process success Domain 4');
        }
      }
    } catch (Exception $e) {
      Log::warning($e->getMessage() . " Domain 4 Send LINE : " . $e->getLine());
    }
  }
}
