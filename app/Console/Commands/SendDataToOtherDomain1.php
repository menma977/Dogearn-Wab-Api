...
class SendDataToOtherDomain1 extends Command
{
  /**
   ...
   * @var string
   */
  protected $signature = 'minute:sendDataToOtherDomain1';
   ...
   * @var string
   */
  protected $description = 'Send Withdraw To domain 1';
    ...
    try {
      ...
      if ($data) {
        ...
        if ($response->successful() && str_contains($response->body(), '1')) {
          ...
          Log::info('delivery process success Domain 1');
        }
      }
    } catch (Exception $e) {
      Log::warning($e->getMessage() . " Domain 1 Send LINE : " . $e->getLine());
    }
  }
}
